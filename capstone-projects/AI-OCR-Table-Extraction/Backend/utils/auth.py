...
if not SECRET_KEY or SECRET_KEY == "your-secret-key-change-this-in-production":
    # For test and local development, avoid raising during import. Use a
    # development key and warn the developer. Production deployments should
    # set a secure `SECRET_KEY` in environment variables.
    import warnings

    warnings.warn(
        "SECRET_KEY is not set or uses the default placeholder. "
        "Falling back to an insecure development key. Set SECRET_KEY in .env for production.",
        UserWarning,
    )
